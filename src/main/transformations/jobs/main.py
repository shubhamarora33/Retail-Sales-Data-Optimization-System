...
from pyspark.sql.functions import concat_ws, current_date, lit, expr
from pyspark.sql import SparkSession
from pyspark.sql.types import StructField, IntegerType, StructType, StringType, DataType, FloatType
from select import select

from resources.dev import config
from resources.dev.config import aws_access_key, aws_secret_key, bucket_name, local_directory, error_folder_path_local, \
    customer_table_name, product_staging_table
...
from src.main.transformations.jobs.sales_mart_sql_transfrom_write import sales_mart_calculation_table_write
...
#Additional columns to be taken care of
# Determine extra columns

#Before running the process
#stage table needs to be updated with the status as Active(A) or Inactive(I)
...
if correct_files:
    ...
    connection = get_mysql_connection()
    cursor = connection.cursor()
    logger.info("********** My SQL connected successfully *********")
...
current_epach = int(datetime.now().timestamp()) * 1000
...
#find out teh total sales done by each sales every month
